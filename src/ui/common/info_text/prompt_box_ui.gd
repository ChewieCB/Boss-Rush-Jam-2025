...
func _ready() -> void:
	for element in elements:
		...
		if element.begins_with("\\n"):
			var newline_container = line_container.instantiate()
			prompt_line_container.add_child(newline_container)
			active_line_container = newline_container
			
			element = element.trim_prefix("\\n")
		
		var prompt_container: HBoxContainer = active_line_container.get_node("HBoxContainer")
