...
static func generate_points_for_circle(circle_position: Vector2, max_gen_radius: float, poisson_radius: float, retries: int, start_point := Vector2.INF, max_points: int = -1) -> PackedVector2Array:
	var sample_region_rect = Rect2(circle_position.x - max_gen_radius, circle_position.y - max_gen_radius, max_gen_radius * 2, max_gen_radius * 2)
	if start_point.x == INF:
		var angle: float = 2 * PI * randf()
		start_point = circle_position + Vector2(cos(angle), sin(angle)) * max_gen_radius * randf()
	elif not Geometry2D.is_point_in_circle(start_point, circle_position, max_gen_radius):
...
static func _generate_points(shape: int, sample_region_rect: Rect2, poisson_radius: float, retries: int, start_pos: Vector2, max_points: int = -1) -> PackedVector2Array:
	...
	while spawn_points.size() > 0:
		if max_points > 0 and spawn_points.size() >= max_points:
			return points
