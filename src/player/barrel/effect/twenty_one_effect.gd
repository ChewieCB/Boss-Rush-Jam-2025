...
func resolve_quick_twenty_one_effect():
	...
	const HEAL_HP_PERC = 0.2
	if rolled_twenty_one_effect == TwentyOneEffectEnum.HEAL_20_PERC_HP:
		GameManager.player.health_component.heal(GameManager.player.health_component.max_health * HEAL_HP_PERC)
		rolled_twenty_one_effect = TwentyOneEffectEnum.NONE
		twenty_one_found = false
		return

	const LOST_CHIP_PERC = 0.2
	const MAX_LOST_CHIP_AMOUNT = 10000
	if rolled_twenty_one_effect == TwentyOneEffectEnum.LOSE_20_PERC_CHIPS:
		var lost_chip_amount = clamp(round(GameManager.player_currency * LOST_CHIP_PERC), 0, MAX_LOST_CHIP_AMOUNT)
		GameManager.player_currency -= lost_chip_amount
		rolled_twenty_one_effect = TwentyOneEffectEnum.NONE
		twenty_one_found = false
		return

	const EXPLOSION_CURRENT_HP_DAMAGE_PERC = 0.15
	if rolled_twenty_one_effect == TwentyOneEffectEnum.EXPLOSION_15_PERC_HP_NON_LETHAL:
		var lost_chip_amount = clamp(round(GameManager.player_currency * LOST_CHIP_PERC), 0, MAX_LOST_CHIP_AMOUNT)
		GameManager.player_currency -= lost_chip_amount
		rolled_twenty_one_effect = TwentyOneEffectEnum.NONE
		twenty_one_found = false
