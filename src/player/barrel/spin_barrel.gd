...
func _record_chosen(idx: int) -> void:
	# When we pick a new effect, move it to the front of the last used queue
	# so we can deprioritise it next spin
	if idx in last_chosen_queue:
		last_chosen_queue.pop_at(last_chosen_queue.find(idx))
	last_chosen_queue.push_front(idx)
	last_chosen_set[idx] = true
...
func stop_spin() -> BaseBarrelEffect:
	is_spinning = false
	...
	_record_chosen(chosen_id)
	
	return get_active_effect()
		...
		match last_chosen_queue.size():
			...
			_:
				...
				if chance <= same_barrel_chance:
					...
					var unused_effects: Array = []
					for idx in range(effect_list.size()):
						if idx not in last_chosen_set:
							unused_effects.append(idx)
